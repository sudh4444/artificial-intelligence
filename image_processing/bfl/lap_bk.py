...
def dynamic_crop(g_bk):
	...
	_,g9 = cv2.threshold(g9,40,255,cv2.THRESH_BINARY)
...
def continuous_gabor(image):
	...
	for theta in np.arange(0,np.pi,np.pi/8):
		kernel = cv2.getGaborKernel((ksize,ksize), 1.2, theta, 5.8, 10, 0, ktype = cv2.CV_32F)
...
if __name__ == "__main__":
	...
	for img in images:
		...
		# clahe = cv2.equalizeHist(original_image)

		if debug:
			showimage("hist",clahe)

		crop_image = dynamic_crop(clahe)
